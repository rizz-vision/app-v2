...
export function WardrobeScreen() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => speak(items.length === 0 ? RESPONSES.wardrobeEmpty : RESPONSES.wardrobeCount(items.length)), 400)
    return () => clearTimeout(t)
  }, [loading]) // eslint-disable-line react-hooks/exhaustive-deps

  const readAll = useCallback(() => {
    if (filtered.length === 0) { speak(RESPONSES.wardrobeEmpty); return }
    speak(`You have ${filtered.length} items. ` + filtered.map((item, i) => `Item ${i + 1}: ${item.name}.`).join(' '))
  }, [filtered, speak])
  ...
  const handleEdit = useCallback((item) => navigate(SCREENS.EDIT_ITEM, { item }), [navigate])
    ...
    return (
      <Screen title="My Wardrobe" subtitle="Your wardrobe is empty.">
        <div style={{ textAlign: 'center', padding: '40px 0' }}>
          <p style={{ fontSize: 16, color: COLORS.TEXT_MUTED, lineHeight: 1.7, marginBottom: 24 }}>
  ...
  return (
    <Screen title="My Wardrobe" subtitle={`${filtered.length} item${filtered.length !== 1 ? 's' : ''}${filter ? ` · ${filter}` : ''}`}>
...
function FilterChip({ label, active, onClick }) {
  return (
    <button onClick={onClick} aria-pressed={active}
      style={{
        background: active ? COLORS.SURFACE_INVERSE : 'transparent',
        color: active ? COLORS.TEXT_ON_ACCENT : COLORS.TEXT,
        border: `2px solid ${active ? COLORS.SURFACE_INVERSE : COLORS.BORDER}`,
...
function LoadingBars() {
  return (
    <div style={{ display: 'flex', gap: 5, alignItems: 'center', height: 40 }}>
      ...
      <style>{`@keyframes lbar { 0%,100%{transform:scaleY(0.3)} 50%{transform:scaleY(1)} }`}</style>
    </div>
  )
}
